...
import club.eridani.cursa.utils.NordTessellator;
import club.eridani.cursa.utils.TotemPopCham;
...
@Module(name = "PopChams", category = Category.RENDER)
public class PopChams extends ModuleBase {
    public Setting<Boolean> self = setting("Self", false);
    public Setting<Integer> rL = setting("RedLine", 255, 0, 255);
    public Setting<Integer> gL = setting("GreenLine", 26, 0, 255);
    public Setting<Integer> bL = setting("BlueLine", 42, 0, 255);
    public Setting<Integer> aL = setting("AlphaLine", 42, 0, 255);

    public Setting<Integer> rF = setting("RedFill", 255, 0, 255);
    public Setting<Integer> gF = setting("GreenFill", 26, 0, 255);
    public Setting<Integer> bF = setting("BlueFill", 42, 0, 255);
    public Setting<Integer> aF = setting("AlphaFill", 42, 0, 255);

    public Setting<Integer> fadestart = setting("FadeStart", 200, 0, 3000);
    public Setting<Double> fadetime = setting("FadeStart", .5, .0,2d);
    public Setting<Boolean> onlyOneEsp = setting("OnlyOneEsp", true);
    ...
    public void onPacketReceived(PacketEvent.Receive event) {
        if (event.getPacket() instanceof SPacketEntityStatus) {
            ...
            if (packet.getOpCode() == 35 && packet.getEntity(mc.world) != null && (self.getValue() || packet.getEntity(mc.world).getEntityId() != mc.player.getEntityId())) {
    ...
    public void renderEntity(final EntityLivingBase entity, final ModelBase modelBase, final float limbSwing, final float limbSwingAmount, final float ageInTicks, final float netHeadYaw, final float headPitch, final float scale) {
    ...
    public void prepareTranslate(final EntityLivingBase entityIn, final double x, final double y, final double z) {
